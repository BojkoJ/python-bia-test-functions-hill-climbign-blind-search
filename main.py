...
def simulated_annealing(
    ...
    """
    Simulated Annealing (minimalizace) + VOLITELNÁ heatmap vizualizace pro 2D.

    Algoritmus:
      1. Začneme s náhodným bodem a vysokou teplotou T_initial
      2. V každé iteraci:
         - Vygenerujeme souseda (Gaussův krok)
         - Pokud je lepší, vždy ho přijmeme
         - Pokud je horší, přijmeme ho s pravděpodobností exp(-ΔE / T)
      3. Teplota se postupně snižuje: T_new = alpha * T_old
      4. Končíme po max_iter iteracích nebo když T < T_min

    Vizualizace (jen 2D):
      - Heatmapa funkce (jako v obrázku)
      - Cesta algoritmu vykreslená čarou s body
      - Start (modrý), konec (zelený)
    ...
    T_initial : float
        Počáteční teplota.
    T_min : float
        Minimální teplota (zastavovací kritérium).
    alpha : float
        Koeficient ochlazování (0 < alpha < 1). Typicky 0.9 - 0.99.
    step_sigma : float
        Směrodatná odchylka pro generování sousedů.
    seed : Optional[int]
    ...
    # Pomocná funkce: vygeneruje náhodný vektor
    def random_vector():
        v = []
        for low, high in bounds:
            value = rng.uniform(low, high)
    ...
    # Pro vizualizaci: ukládáme celou cestu (všechny akceptované kroky)
    path: List[Tuple[List[float], float]] = [(list(current), current_f)]

    # 2) Hlavní smyčka
    it = 0
    while it < max_iter and T > T_min:
        # Generujeme souseda pomocí Gaussova šumu
        neighbor = []
        for i in range(dim):
            low, high = bounds[i]
            step = rng.gauss(0.0, step_sigma)
            cand = current[i] + step
            cand = clamp(cand, low, high)
            neighbor.append(cand)

        f_nei = objective(neighbor)
        delta_E = f_nei - current_f

        # Rozhodnutí o přijetí souseda
        accept = False
        if delta_E < 0:
            # Lepší řešení -> vždy přijmeme
            accept = True
        else:
            # Horší řešení -> přijmeme s pravděpodobností exp(-ΔE / T)
        ...
        if accept:
            current = neighbor
            current_f = f_nei
            path.append((list(current), current_f))
    ...
    if visualize and dim == 2:
        ...
        # Heatmapa: použijeme contourf (vyplněné kontury) nebo imshow
        # contourf je lepší pro hladký vzhled podobný obrázku
        levels = 50  # Počet úrovní v heatmapě
        contour = ax.contourf(X, Y, Z, levels=levels, cmap='jet')
        
        # Přidáme colorbar (barevnou škálu)
        ...
        ax.set_xlabel('x1')
        ax.set_ylabel('x2')
        ax.set_title('Simulated Annealing – Heatmap')

        # Vykreslíme cestu algoritmu
        if len(path) > 1:
            path_x = [state[0][0] for state in path]
            path_y = [state[0][1] for state in path]
            
            # Cesta jako čára
            ax.plot(path_x, path_y, 'w-', linewidth=1.5, alpha=0.7, label='SA path')
        ...
        msg = f"SA: best=({best_x[0]:.4f}, {best_x[1]:.4f}), f={best_f:.6g}, steps={len(path)}"
        print(msg)
